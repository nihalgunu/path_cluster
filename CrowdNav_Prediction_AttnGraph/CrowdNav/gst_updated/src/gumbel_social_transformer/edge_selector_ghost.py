...
from src.gumbel_social_transformer.group_generator import GroupGenerator
...
class EdgeSelector(nn.Module):
    ...
    def __init__(self, d_motion, d_model, nhead=4, dropout=0.1, activation="relu"):
        ...
        self.group_gen = GroupGenerator(d_type='learned_l2norm')  # Instantiate the GroupGenerator class

    def forward(self, x, A, attn_mask, tau=1., hard=False, device='cuda:0'):
        A = self.group_gen(x, attn_mask, tau=tau, hard=hard)  # Use GroupGenerator to group individuals

        bsz, nnode, d_motion = v_abs.shape
