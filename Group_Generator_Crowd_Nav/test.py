...
def main():
	...
	env_config.render_traj = True
	env_config.save_slides = True
	env_config.save_path = os.path.join(test_args.model_dir, 'eval', test_args.test_model[:-3])
	print("this config is:")
	print(env_config.save_slides)
	...
	evaluate(actor_critic, envs, 1, device, test_size, logging, config, algo_args, True)
